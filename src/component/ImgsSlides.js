...
export default function ImgsSlides() {
  ...
  return (
    <div id="window" style={{backgroundImage:`url(/images/${findeImage()})`}}>
        <div id="conteneur">
        ...
        <div id="imgPosition">
            {imgs.map(
                function (src, index) {
                    ...
                    return <span key={src.id} className="daeira"><div style={{display:disp}} className='selected' id={`selected${src.id}`}></div></span>
                }
            )}
        </div>
    </div>
